...
import { Alert, Button, message, Segmented } from 'ant-design-vue';
...
>([]);
const assetType = ref<'security_device' | 'server' | 'switch'>('server');
...
const [BaseForm, formApi] = useVbenForm({
  ...
  schema: [
    ...
    },
    {
      ...
      defaultValue: false,
      fieldName: 'isAddRun',
      help: '仅添加：只保存服务器资产；添加&获取：保存后立即提交一次硬件采集任务',
      label: '操作',
      rules: z.boolean(),
    },
    {
...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    if (assetType.value !== 'server') {
      message.warning('当前资产类型尚未接入添加表单');
      return;
    }
    if (submitting.value) {
      return;
  ...
  async onOpenChange(isOpen: boolean) {
    if (isOpen) {
      ...
      submitting.value = false;
      submittedResponse.value = undefined;
      assetType.value = values.asset_type || 'server';
      modalApi.setState({ confirmLoading: false });
  ...
  <Modal>
    <div class="asset-add-form">
      <Segmented
        ...
        block
        class="asset-type-segmented"
        :options="assetTypeOptions"
      />
      <div v-if="assetType === 'server'" class="server-add-form">
        <BaseForm />
